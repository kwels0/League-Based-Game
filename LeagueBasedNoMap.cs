...
[4] Realm of Death",1);
        ...
        public int Indestructible(enemyStats enemy)
        {
            if (Stamina >= 25)
            {
        ...
        public int DeathsGrasp(enemyStats enemy)
        {
            if (Stamina >= 25)
            {
                //enemyStats.Spd -= 10;
        ...
        public int RealmofDeath(enemyStats enemy)
        {
            if (Stamina >= 50)
            {
                Hp += 5;
    ...
    {
        public static Mordekaiser SpawnMordekaiser()
        {
    ...
    public class Jhin : ChampionStats
    {
        ...
        public Jhin(
            string name = "Jhin, the Virtuoso",
            string job = "Marksman",
            string description = "Jhin is a meticulous criminal psychopath who believes murder is art. Once an Ionia Crest icon Ionian prisoner, but freed by shadowy elements within Ionia's ruling council, the serial killer now works as their cabal's assassin. Using his gun as his paintbrush, Jhin creates works of artistic brutality, horrifying victims and onlookers. He gains a cruel pleasure from putting on his gruesome theater, making him the ideal choice to send the most powerful of messages. Terror.",
            double hp = 655,
            int maxHP = 655,
            double dmg = 59,
            int spd = 30,
...
[4] Curtain Call",1);
            ...
            switch(num)
            {
                ...
                case '2':
                {
                    DeadlyFlourish(enemy);
                    return;
                }
                case '3':
                {
                    CaptiveAudience(enemy);
                    return;
                }
                case '4':
                {
        ...
        public int Innate(enemyStats enemy)
        {   
            if (ArrowCount >= 1)
            {
                double damage = Dmg * 0.20;
                ...
                int arrow = 1;
                ArrowCount -= arrow;
                Console.WriteLine($"You used Innate and dealt {damage} damage!");
                return ArrowCount; 
            }
            else
            {
                delay($"You do not have enough stamina to use Innate!",1);
                return ArrowCount; 
            }
        }
        public int DeadlyFlourish(enemyStats enemy)
        {
            if (ArrowCount >= 1)
            {
                double damage = Dmg * 0.20;
                ...
                int arrow = 1;
                ArrowCount -= arrow;
                Console.WriteLine($"You used Deadly Flourish and dealt {damage} damage!");
                return ArrowCount; 
            }
            else
            {
                delay($"You do not have enough stamina to use Deadly Flourish!",1);
                return ArrowCount; 
            }
        }
        public int CaptiveAudience(enemyStats enemy)
        {
            if (ArrowCount >= 1)
            {
                double damage = Dmg * 0.20;
        ...
        public int CurtainCall(enemyStats enemy)
        {
    ...
    public class JhinCreate
    {
        public static Jhin SpawnJhin()
        {
            return new Jhin();
        }
    }
...
[4] Crowstorm",1);
        ...
        public int Terrify(enemyStats enemy)
        {
            if (Mp >= 25)
            {
                double damage = Dmg * 0.20;
        ...
        public int BountifulHarvest(enemyStats enemy)
        {
            if (Mp >= 25)
            {
                double damage = Dmg * 0.20;
        ...
        public int Reap(enemyStats enemy)
        {   
            if (Mp >= 25)
            {
                double damage = Dmg * 0.20;
        ...
        public int Crowstorm(enemyStats enemy)
        {
            if (Mp >= 25)
            {
                double damage = Dmg * 0.20;
    ...
    public class FiddleCreate
    {
        public static Fiddlesticks SpawnFiddlesticks()
        {
            return new Fiddlesticks();
        }
    }
    ...
    public class Warwick : ChampionStats
    {
        ...
        public void JawsOfTheBeast(enemyStats enemy)
        {
            double damage = Dmg * 0.20;
            enemy.Hp -= damage; 
            Console.WriteLine($"You used Obliterate and dealt {damage} damage!");
        ...
        public void BloodHunt(enemyStats enemy)
        {
            //Spd += 10;
            //delay("Increase speed for 10",1);
        }
        public void PrimalHowl(enemyStats enemy)
        {
            //delay("Recieve less damage, Stun enemy next round",1);
        }
        public void InfiniteDuress(enemyStats enemy)
        {
    ...
    public class Cho_gath : ChampionStats
    {
        ...
        public void Rupture(enemyStats enemy)
        {
            //damg and stun enemy
        }
        public void FeralScream(enemyStats enemy)
        {
            //dmg and stun enemy but unable to move instead for write
        }
        public void VorpalSpikes(enemyStats enemy)
        {
            //attack enemy 3 times
        }
        public void Feast(enemyStats enemy)
        {
...
\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `'\ `' /
 `--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--'`--' ");
        ...
        keyRead();
        Mordekaiser mordekaiser = MordCreate.SpawnMordekaiser();
        ShadowIsles.spectralIronhound spectralIronhound = new ShadowIsles.spectralIronhound();
